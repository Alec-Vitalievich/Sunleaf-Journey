...
void Game::load_level(bool *new_level)
{
    ...
    {
        if (level)
        { // Checks if the pointer is not null. level != nullptr could be used for clarity?
            delete level;
            level = nullptr;
        }
        *new_level = false;
...
void Game::update()
{
    ...
    while (window.isOpen())
    {
        ...
        else if (current_state == GameState::PLAYING)
        {
            ...
            if (dt > 0.2)
            {             // Prevents delta time from becoming too large. (dt checks time since last frame - if the window was paused for instance, this could become huge otherwise).
                dt = 0.2; // We may want to play with this value a bit.
            }
            else if (dt < 0.0)
